...
public class TicketDAO { 
            ...
            if(rs.next()){
              ...
              //  ticket.setOutTime(rs.getTimestamp(5).toLocalDateTime());
                ...
                if (timestamp != null) {
                    ...
                    ticket.setOutTime(localDateTime);
                }else
                {
    ...
    public boolean verifyVehicleRegNumber(String vehicleRegNumber )
    {
        ...
        try {
            con = dataBaseConfig.getConnection();
            PreparedStatement ps = con.prepareStatement(DBConstants.VERIFY_VEHICLEREGNUMBER);
            ps.setString(1,vehicleRegNumber);
            ResultSet rs = ps.executeQuery();
            if(rs.next()){
               
               System.out.println("temps de sortie ====> " + rs.getTimestamp(6));
